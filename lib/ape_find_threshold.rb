...
module Ape
  def self.run_find_threshold(model_filename, pvalue, **kwargs)
    run_find_thresholds(model_filename, [pvalue], **kwargs)[pvalue]
  end

  def self.run_find_thresholds(model_filename, pvalues,
                          background: nil,
                          discretization: nil,
                          additional_options: [],
                          mode: :mono)
